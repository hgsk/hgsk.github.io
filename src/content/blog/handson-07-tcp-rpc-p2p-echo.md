...
*フレームワークに頼らず、TCP上に<punch-line>最小限のRPC</punch-line>を自力で実装してみます。
プロトコルが何をしているかを理解するための単元です。
フレームワークは理解を先送りにする装置でもあるので、今回はその装置を使わない方針です。*
...
*このコードをそのまま本番に持ち込む前に、<punch-line>最低限ここだけは対処</punch-line>しておきたいところです。
動くコードと、本番で動き続けるコードの間には、いくつかの深い溝がある気がします。
ここで挙げるのは、その溝のうちの最小の3つです。*
